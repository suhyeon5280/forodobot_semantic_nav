...
async def _gpsimu_poll_loop():
    """Background task: logs GPS/IMU at its natural ~1Hz rate, independent of camera/control."""
    try:
        while recording_state["active"]:
            try:
                data = await browser_service.data()
                if data:
                    record = {"local_timestamp": datetime.utcnow().timestamp(), **data}
                    recording_state["gpsimu_log"].write(json.dumps(record) + "\n")
                    recording_state["gpsimu_log"].flush()
                    recording_state["gpsimu_count"] += 1
            except Exception as e:
                logger.error("gps_imu poll failed: %s", e)
    ...
    recording_state.update(
        {
            ...
            "control_log": open(os.path.join(session_dir, "control.jsonl"), "a"),
            "gpsimu_log": open(os.path.join(session_dir, "gps_imu.jsonl"), "a"),
            "frame_count": 0,
            "gpsimu_count": 0,
        }
    )
    recording_state["gpsimu_task"] = asyncio.create_task(_gpsimu_poll_loop())

    return {"session_name": session_name, "session_dir": session_dir}


@app.post("/dataset/log-frame")
async def dataset_log_frame(body: LogFrameRequest):
    if not recording_state["active"]:
        raise HTTPException(status_code=400, detail="No active recording session")

    image_bytes = base64.b64decode(body.image_base64)
    image_rel_path = os.path.join("images", f"{body.timestamp:.3f}.jpg")
    image_path = os.path.join(recording_state["session_dir"], image_rel_path)
    with open(image_path, "wb") as f:
        f.write(image_bytes)

    record = {
        "image": image_rel_path,
        "local_timestamp": body.timestamp,
        "linear": body.linear,
        "angular": body.angular,
    }
    recording_state["control_log"].write(json.dumps(record) + "\n")
    recording_state["control_log"].flush()
    recording_state["frame_count"] += 1

    return {"frame_count": recording_state["frame_count"]}


@app.post("/dataset/stop")
async def dataset_stop():
    if not recording_state["active"]:
        raise HTTPException(status_code=400, detail="No active recording session")

    recording_state["active"] = False
    if recording_state["gpsimu_task"]:
        recording_state["gpsimu_task"].cancel()

    recording_state["control_log"].close()
    recording_state["gpsimu_log"].close()

    summary = {
        "session_name": recording_state["session_name"],
        "session_dir": recording_state["session_dir"],
        "frames": recording_state["frame_count"],
        "gps_imu_samples": recording_state["gpsimu_count"],
    }
...
async def dataset_status():
    return {
        ...
        "gpsimu_count": recording_state["gpsimu_count"],
    }
