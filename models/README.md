...
`hf/`는 심볼릭 링크를 이미 풀어둔 상태입니다. Hugging Face 캐시는 원래 `blobs/`의
실제 파일을 `snapshots/`에서 링크로 가리키는데, FAT32/exFAT USB는 링크를 저장하지
못해 복사가 멈춥니다. 캐시를 새로 만들었다면 `cp -rL`로 풀고 `blobs/`를 지우세요.
